...
function bubble() {
  for (var i = 1; i < bubble_artefact.numOfCards; i++) {
    if (eval(bubble_artefact.card[i - 1].innerHTML) > eval(bubble_artefact.card[i].innerHTML)) {
...
function check_swap() {
  ...
  } else document.getElementById('ins').innerHTML = 'Incorrect, Think again!'
  // if (n != bubble_artefact.numOfCards - 1) bubble_artefact.finished = false
  // else bubble_artefact.finished = true
}
...
function check_next() {
  ...
  if (temp == 1)
    document.getElementById('ins').innerHTML =
      'Incorrect, Think again! Do we need to swap here?'
  else {
    document.getElementById('ins').innerHTML = 'Correct!'
    window[bubble_artefact.fn_name]()
    compare(bubble_artefact.iterator1, bubble_artefact.iterator2)
