...
function change_interval() {
  if (
    ...
    (bubble_artefact.action == 1 || bubble_artefact.action == -1)
  ) {
    ...
    if (document.getElementById("interval").value != 100) {
      if (bubble_artefact.fn_name > "")
        bubble_artefact.interval = setInterval(
          next_step,
          2600 - document.getElementById("interval").value
        );
...
function compare(i, j) {
  ...
  for (var n = 0; n < bubble_artefact.numOfCards; n++) {
    if (n == i || n == j) {
      if (
        bubble_artefact.card[n].style.backgroundColor != "grey" ||
        !bubble_artefact.flag
      )
  ...
  document.getElementById("ins").innerHTML =
    ...
    "</p><p>Comparing " +
    bubble_artefact.card[i].innerHTML +
    " and " +
    bubble_artefact.card[j].innerHTML +
    "...</p>";
  if (
    eval(bubble_artefact.card[j].innerHTML) <
    eval(bubble_artefact.card[i].innerHTML)
  ) {
    document.getElementById("ins").innerHTML +=
      "<p>" +
      bubble_artefact.operation +
      " required" +
      " : " +
      "Swapping " +
      bubble_artefact.card[i].innerHTML +
    ...
    document.getElementById("ins").innerHTML +=
      "<p>" + bubble_artefact.operation + " not required</p>";
    return false;
  }
}
...
function swap(i, j) {
  bubble_artefact.swaps++;
  bubble_artefact.swappedInPass = true;
  var temp;
...
function opti_bubble() {
  if (bubble_artefact.iterator1 < bubble_artefact.end) {
    ...
    // End of pass reached
    bubble_artefact.passes++;
    bubble_artefact.card[bubble_artefact.end + 1].style.backgroundColor =
      "grey";

    if (!bubble_artefact.swappedInPass) {
      // No swaps in this pass - array is sorted, optimization triggered!
      if (document.getElementById("interval").value != 100) {
      ...
      document.getElementById("ins").innerHTML =
        ...
        " swaps in " +
        bubble_artefact.passes +
        " passes.</h3><p>Optimization: Algorithm stopped early because no swaps were needed in the last pass!</p>";
    ...
    } else if (bubble_artefact.end > 0) {
      // Continue to next pass
      bubble_artefact.swappedInPass = false;
